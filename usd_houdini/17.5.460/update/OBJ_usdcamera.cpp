...
/* *****************************************************************************
USD Camera for Dexter
$1: (charles) - 2D Pan/Zoom
***************************************************************************** */
...
GusdOBJ_usdcamera::GetVariables()
{
    static CH_LocalVariable vars[] = {
        ...
        // $1: 2D Pan/Zoom Variables
        {"PANX", VAR_PANX, CH_VARIABLE_TIME},
        {"PANY", VAR_PANY, CH_VARIABLE_TIME},
        {"ZOOM", VAR_ZOOM, CH_VARIABLE_TIME},
        //
...
GusdOBJ_usdcamera::runCreateScript()
{
    static _NameDefaultPair pairs[] = {
        // $1: chage camera size 500 to 50 (500 is too big)
        {"iconscale", 0, PRM_Default(50,"")},
        ...
        // $1: set param for 2D Pan/Zoom
        //{"win", 0, PRM_Default(0, "$HAPERTUREOFFSET/$HAPERTURE")},
        {"win", 0, PRM_Default(0, "$PANX*(25.4/$HAPERTURE)")},
        {"win", 1, PRM_Default(0, "$PANY*(25.4/$VAPERTURE)")},
        {"winsize", 0, PRM_Default(1, "$ZOOM")},
        {"winsize", 1, PRM_Default(1, "$ZOOM")},
        //
...
GusdOBJ_usdcamera::evalVariableValue(fpreal& val, int idx, int thread)
{
    ...
    if(HBOOST_UNLIKELY(!stack)) stack = new _VarEvalStack;
...
GusdOBJ_usdcamera::_EvalCamVariable(fpreal& val, int idx, int thread)
{
    ...
    if(UsdGeomCamera cam = _LoadCamera(t, thread))
    {
        ...
        case VAR_PANX:
            {
                UsdPrim camPrim = cam.GetPrim();
                float panx = 0.0;
                TfToken attrName = TfToken("userProperties:Camera:PanZoom:Pan");
                TfToken offsetAttrName = TfToken("horizontalApertureOffset");
                if(UsdAttribute attr = camPrim.GetAttribute(offsetAttrName))
                {
                    attr.Get(&panx, frame);
                    panx /= 25.4;
                }
                else if(UsdAttribute attr = camPrim.GetAttribute(attrName))
                {
                    GfVec2f pan;
                    panx = attr.Get(&pan, frame) ? pan[0] : 0.0;
                }
                val = panx;
                return true;
            }
        case VAR_PANY:
            {
                ...
                float panx = 0.0;
                TfToken attrName = TfToken("userProperties:Camera:PanZoom:Pan");
                TfToken offsetAttrName = TfToken("verticalApertureOffset");
                if(UsdAttribute attr = camPrim.GetAttribute(offsetAttrName))
                {
                    attr.Get(&panx, frame);
                    panx /= 25.4;
                ...
                {
                    GfVec2f pan;
                    panx = attr.Get(&pan, frame) ? pan[1] : 0.0;
                }
                val = panx;
                return true;
            }
        case VAR_ZOOM:
            {
                UsdPrim camPrim = cam.GetPrim();
                float zoom = 1.0;
                TfToken attrName = TfToken("userProperties:Camera:PanZoom:Zoom");
                if(UsdAttribute attr = camPrim.GetAttribute(attrName))
                {
                    attr.Get(&zoom, frame);
                }
                val = zoom;
    ...
    // $1: 2D Pan/Zoom
    case VAR_PANX:                  val = 0.0; break;
    case VAR_PANY:                  val = 0.0; break;
    case VAR_ZOOM:                  val = 1.0; break;
    };
