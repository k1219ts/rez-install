        ...
        const std::string& outputImagePath)
{
    ...
    // 2019.07.17 daeseok.chae modified
    ...
    const GfVec3d cam_pos = frustum.GetPosition();
    const GfVec3d cam_up = frustum.ComputeUpVector();
    const GfVec3d cam_right = GfCross(frustum.ComputeViewDirection(), cam_up);
    const float cam_light_dist = 50;

    // key Light
    const GfVec3d keyHorz = -1 / tan(GfDegreesToRadians(45)) * cam_right;
    const GfVec3d keyVert = 1 / tan(GfDegreesToRadians(70)) * cam_up;
    const GfVec3d keyPos = cam_pos + (keyVert + keyHorz) * cam_light_dist;
    const GfVec4f keyColor(0.8f, 0.8f, 0.8f, 1.0f);

    GlfSimpleLight keyLight;
    keyLight.SetAmbient({0, 0, 0, 0});
    keyLight.SetDiffuse(keyColor);
    keyLight.SetSpecular(keyColor);
    keyLight.SetPosition({keyPos[0], keyPos[1], keyPos[2], 1.0f});

    // fill Light
    const GfVec3d fillHorz = 1 / tan(GfDegreesToRadians(30)) * cam_right;
    const GfVec3d fillVert = 1 / tan(GfDegreesToRadians(45)) * cam_up;
    const GfVec3d fillPos = cam_pos + (fillVert + fillHorz) * cam_light_dist;
    const GfVec4f fillColor(0.6f, 0.6f, 0.6f, 1.0f);

    GlfSimpleLight fillLight;
    fillLight.SetAmbient({0, 0, 0, 0});
    fillLight.SetDiffuse(fillColor);
    fillLight.SetSpecular(fillColor);
    fillLight.SetPosition({fillPos[0], fillPos[1], fillPos[2], 1.0f});

    // back Light
    const GfVec3d origin(0.0);
    GfVec3d backPos = cam_pos + (origin - cam_pos) * 2;
    const GfVec3d backHorz = 1 / tan(GfDegreesToRadians(60)) * cam_right;
    const GfVec3d backVert = -1 / tan(GfDegreesToRadians(60)) * cam_up;
    backPos += (backHorz + backVert) * cam_light_dist;
    const GfVec4f backColor(0.6f, 0.6f, 0.6f, 1.0f);

    GlfSimpleLight backLight;
    backLight.SetAmbient({0, 0, 0, 0});
    backLight.SetDiffuse(backColor);
    backLight.SetSpecular(backColor);
    backLight.SetPosition({backPos[0], backPos[1], backPos[2], 1.0f});

    const GlfSimpleLightVector lights({keyLight, fillLight, backLight});
