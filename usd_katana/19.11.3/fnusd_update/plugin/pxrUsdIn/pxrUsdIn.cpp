...
public:
    ...
    static void cook(FnKat::GeolibCookInterface &interface)
    {
        ...
        if (processStageWideQueries.isValid() &&
            ...
            FnKat::StringBuilder materialListBuilder;
            SdfPathVector materialPaths = PxrUsdKatanaUtils::FindMaterialPaths(stage, opArgs);
            TF_FOR_ALL(materialPathIt, materialPaths)
            {
                const std::string path = (*materialPathIt).GetString();
                if (path.find(usdInArgs->GetIsolatePath()) != std::string::npos)
                {
                    materialListBuilder.push_back(
                        TfNormPath(PxrUsdKatanaUtils::ConvertUsdPathToKatLocation(*materialPathIt, usdInArgs))
                    );
                }
            }

            FnKat::StringAttribute materialListAttr = materialListBuilder.build();
            if (materialListAttr.getNumberOfValues() > 0)
            {
                interface.setAttr("materialList", materialListAttr);
            }
            // -----------------------------------------------------------------
