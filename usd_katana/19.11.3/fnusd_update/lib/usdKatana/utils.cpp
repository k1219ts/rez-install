            ...
            if (master.IsValid())
            {
                ...
                if (masterToKey.find(masterPath) == masterToKey.end())
                {
                    ...
                    if (assetName.empty())
                    {
                        ...
                        {
                            const std::string & variantName = (*I);
                            std::string variantValue = variantSets.GetVariantSet(variantName).GetVariantSelection();
                            buffer << "__" << variantName << "_" << variantValue;
                        }
                    ...
                    {
                        masterToKey[masterPath] = assetName;
                        keyToMasters[assetName].insert(masterPath);
                    }
        ...
        const UsdStageRefPtr& stage, const SdfPath &rootPath)
{
    ...
    TF_FOR_ALL(I, keyToMasters)
    {
        ...
        if (!masters.empty())
        {
            if (masters.size() == 1)
            {
                const std::string & master = (*masters.begin());
                gb.set(FnKat::DelimiterEncode(master), FnKat::StringAttribute(key));
            }
            else
            {
                size_t i = 0;
                TF_FOR_ALL(J, masters)
                {
                    ...
                    std::ostringstream buffer;
                    buffer << key << "/m" << i;
                    gb.set(FnKat::DelimiterEncode(master), FnKat::StringAttribute(buffer.str()));
                    ++i;
                }
            }
        }
...
// Add materialList : rmantd@2020.07.09 ----------------------------------------
namespace
{
    void _GetPrmanMaterialPathFromStage(const UsdStageRefPtr& stage, const UsdPrim& root, SdfPathVector& result)
    {
        for (UsdPrim prim: UsdPrimRange::AllPrims(root))
        {
            if (prim.IsA<UsdShadeMaterial>()) {
                SdfPath primPath = prim.GetPath();
                if (primPath.GetString().find("/prman/") != std::string::npos) {
...
SdfPathVector
PxrUsdKatanaUtils::FindMaterialPaths(const UsdStageRefPtr& stage, const FnKat::GroupAttribute& opArgs)
{
    SdfPathVector result;

    _GetPrmanMaterialPathFromStage(stage, stage->GetDefaultPrim(), result);     // or stage->GetPseudoRoot()

    if (FnAttribute::StringAttribute(opArgs.getChildByName("instanceMode")).getValue("expaned", false) == "as sources and instances")
    {
        StringMap masterToKey;
        StringSetMap keyToMasters;
        _walkForMasters(stage->GetPrimAtPath(SdfPath::AbsoluteRootPath()), masterToKey, keyToMasters);
        ...
        TF_FOR_ALL(I, keyToMasters)
        {
            ...
            if (!masters.empty())
            {
                ...
                {
                    const std::string & master = (*masters.begin());
                    SdfPath masterPath(master);
                    SdfPathVector instmtls;
                    _GetPrmanMaterialPathFromStage(stage, stage->GetPrimAtPath(masterPath), instmtls);
                    TF_FOR_ALL(it, instmtls)
                    {
                        const std::string path = (*it).MakeRelativePath(masterPath).GetString();
                        std::ostringstream buffer;
                        buffer << "/Masters/" << key << "/" << path;
                        result.push_back(SdfPath(buffer.str()));
                    }
                }
                else
                {
